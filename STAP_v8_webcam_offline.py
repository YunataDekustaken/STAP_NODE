...
CHANGES FROM v11.2:
  ...
  │  FIX: Diverted webcam streams to local Toycar testing video paths.   │
  │       Webcam capture structures are retained inside code comments.  │
  └─────────────────────────────────────────────────────────────────────┘
"""
...
# --- WEBCAM CONFIGURATION (COMMENTED OUT) ---
# CAMERA_INDICES = [0, 1, 2, 3] 
# RECONNECT_TIMEOUT = 3.0

# --- VIDEO FILE TESTING CONFIGURATION ---
LOOP_VIDEOS = True
VIDEO_FILES = [
    r"C:\Users\Raphael\Desktop\YOLO\FINAL\Toycar\NORTH.mp4",
    r"C:\Users\Raphael\Desktop\YOLO\FINAL\Toycar\SOUTH.mp4",
    r"C:\Users\Raphael\Desktop\YOLO\FINAL\Toycar\EAST.mp4",
    r"C:\Users\Raphael\Desktop\YOLO\FINAL\Toycar\WEST.mp4"
]
...
# =============================================================
# 4. INITIALIZE VIDEO FILE CAPTURES (WEBCAM FALLBACK LAYER)
# =============================================================
caps = []
print("[STAP] Opening video streams and calibrating aspect boundaries...")

# --- OLD WEBCAM INITIALIZATION (COMMENTED OUT) ---
...
for idx, path in enumerate(VIDEO_FILES):
    ...
    if hw_w <= 0 or hw_h <= 0:
        hw_w, hw_h = 1920.0, 1080.0 # Fallback resolution if video property cannot be parsed
    
    # Calculate perfect downscaling translation from your high-res coordinates
    ...
    print(f"[STAP] Calibrated {lane} ROI matrix using video frame limits ({int(hw_w)}x{int(hw_h)}).")
...
class BackgroundAIProcessor(threading.Thread):
    def __init__(self, model_path, device):
        ...
        if device != "cpu":
            ...
            print(f"[STAP] ✅ Unified Single-Pass Layout Tracker initialized on GPU.")
        else:
            print("[STAP] ⚠️ YOLO model running on CPU.")
        ...
        while self.running:
            ...
            with frame_lock:
                for idx in range(4):
                    if latest_frames[idx] is not None:
                        imgs[idx] = latest_frames[idx].copy()

            if any(f is None for f in imgs):
                time.sleep(0.01)
                continue

            # Stitch separate lane arrays into a single unified 2x2 layout frame
            top_row = np.hstack((imgs[0], imgs[1]))      # North | South
            bottom_row = np.hstack((imgs[2], imgs[3]))   # East  | West
            stitched_grid = np.vstack((top_row, bottom_row))
            ...
            try:
                r = self.model.track(
                    ...
                    verbose=False, 
                    device=self.device, 
                    imgsz=1280, # Match unified composite pixel boundaries (640*2)
                    half=self.half
                )
                res = r[0]
            except Exception as e:
                print(f"[STAP] Grid execution drop: {e}")
                continue
            ...
            for box in res.boxes:
                ...
                # Segment coordinates back into target lane parameters
                if gcx < CAM_WIDTH and gcy < CAM_HEIGHT:
                    lane_idx, lane = 0, "NORTH"
                    lx1, ly1, lx2, ly2 = gx1, gy1, gx2, gy2
                elif gcx >= CAM_WIDTH and gcy < CAM_HEIGHT:
                    lane_idx, lane = 1, "SOUTH"
                    lx1, ly1, lx2, ly2 = gx1 - CAM_WIDTH, gy1, gx2 - CAM_WIDTH, gy2
                elif gcx < CAM_WIDTH and gcy >= CAM_HEIGHT:
                    lane_idx, lane = 2, "EAST"
                    lx1, ly1, lx2, ly2 = gx1, gy1 - CAM_HEIGHT, gx2, gy2 - CAM_HEIGHT
                else:
                    lane_idx, lane = 3, "WEST"
                    lx1, ly1, lx2, ly2 = gx1 - CAM_WIDTH, gy1 - CAM_HEIGHT, gx2 - CAM_WIDTH, gy2 - CAM_HEIGHT
...
try:
    ...
    print(f"[STAP] ❌ Serial connection failed ({e}). Running simulation mode.")
    ser = None
...
def control_light():
    if request.method == 'OPTIONS': return jsonify({}), 200
    if not manual_override: return jsonify({'success': False, 'message': 'Set manual mode first.'}), 422
    ...
    if lane not in LANE_NAMES or state not in ['red', 'yellow', 'green']:
        return jsonify({'success': False, 'message': 'Invalid lane/state parameters.'}), 400
...
def control_emergency():
    ...
    if lane not in LANE_NAMES: return jsonify({'success': False, 'message': 'Invalid lane.'}), 400
...
def get_status():
    ...
    return jsonify({
        ...
        'green_duration': green_dur, 'mode': 'manual' if manual_override else 'auto',
        'rain': rain_detected, 'vehicle_counts': counts, 'los': los_per_lane, 'lane_statuses': statuses,
    })
...
# Launch Core Background Processes
...
print("[STAP] ✅ Per-lane streams active on LAN Port 5000")
...
while True:
    ...
    # --- READ ALL SOURCE FEEDS INLINE (VIDEO LOOP FALLBACK REPLACING WEBCAMS) ---
    current_frames = []
    for i, cap in enumerate(caps):
        ret, frame = cap.read()
        
        # --- ORIGINAL WEBCAM CAPTURE HANDLER CODE (COMMENTED OUT) ---
        # if not ret:
        ...
        # --- VIDEO FILE INPUT ROUTING AND AUTO-LOOPING HANDLING LAYER ---
        if not ret:
            if LOOP_VIDEOS:
    ...
    # Multi-cast downscaled lane views out to active endpoints
    for idx, lane in enumerate(LANE_NAMES):
